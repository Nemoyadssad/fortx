...
import { ArrowLeft, Radio, Check, ChevronUp } from 'lucide-react';
...
export default function EventDetailPage() {
  ...
  // Lock scroll when sheet is open on mobile
  useEffect(() => {
    if (sheetOpen) {
  ...
  // Shared bet panel content
  const BetPanelContent = (
    <>
      ...
      <div className="mt-2 grid grid-cols-4 gap-2">
        {CHIPS.map((v) => (
          <button
            key={v}
            onClick={() => setStake(v)}
            className="rounded-lg border hairline py-2 text-sm font-medium text-fg/60 transition hover:border-gold/40 hover:text-gold-deep active:scale-95"
          >
            {v}
          </button>
        ))}
      ...
      <button
        ...
        className="mt-3 w-full rounded-xl bg-gradient-to-b from-gold to-gold-soft py-3.5 font-bold text-black shadow-gold transition hover:brightness-105 active:scale-[0.98] disabled:opacity-50"
      >
  ...
  return (
    <>
      {/* Main page — add bottom padding on mobile so content isn't hidden behind sticky bar */}
      <div className="mx-auto max-w-6xl px-4 py-6 pb-28 lg:pb-6">
        ...
        <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_340px]">
          ...
          <div className="min-w-0 space-y-5">
            ...
            <div className="flex items-start gap-3">
              {event.imageUrl ? (
                ...
                <img
                  ...
                  className="h-12 w-12 shrink-0 rounded-2xl object-cover lg:h-14 lg:w-14"
                />
              ) : (
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gold/15 font-display text-xl font-bold text-gold-deep lg:h-14 lg:w-14">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 text-[11px] text-fg/45">
                  {event.category && (
                    <span className="rounded bg-fg/[0.05] px-1.5 py-0.5 font-mono uppercase tracking-wider">
                      {event.category}
                    </span>
                  )}
                ...
                <h1 className="mt-1 line-clamp-3 font-display text-xl font-bold leading-tight lg:text-2xl">
                  {event.title}
                </h1>
              </div>
            </div>
            ...
            {/* Chart */}
            <div className="rounded-2xl panel p-4 lg:p-5">
              <p className="mb-2 text-sm font-semibold text-fg/80">{mkt.question}</p>
              {series.length > 0 ? (
                <PriceChart series={series} />
              ) : (
                <div className="flex h-40 items-center justify-center text-xs text-fg/30">
            ...
            {event.markets.length > 1 && (
              <div className="rounded-2xl panel">
                <div className="border-b hairline px-4 py-3 lg:px-5">
                  ...
                  {event.markets.map((m) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition hover:bg-fg/[0.03] lg:px-5 ${m.id === mkt.id ? 'bg-gold/[0.05]' : ''}`}
                      >
                        <span className="min-w-0 truncate text-sm text-fg/80">{m.question}</span>
                        <span className="shrink-0 font-mono text-sm font-bold text-gold-deep">
                          {top ? `${pct(top.price)}%` : '—'}
                        </span>
                      </button>
                    );
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden">
        ...
        {/* Bottom sheet */}
        <div
          className={`relative rounded-t-2xl panel border-t hairline shadow-2xl transition-transform duration-300 ease-in-out ${
            sheetOpen ? 'translate-y-0' : 'translate-y-[calc(100%-72px)]'
          }`}
        >
          {/* Collapsed bar — always visible, tap to open */}
          <button
              ...
              {selectedOutcome ? (
                <>
                  <span className={`rounded-lg px-2.5 py-1 text-sm font-bold ${
                    binary
                      ? binary.yes.id === selectedOutcome.id
                        ? 'bg-win/15 text-win'
                  ...
                  <span className="font-mono text-sm text-fg/60">{pct(selectedOutcome.price)}%</span>
                </>
              ) : (
                <span className="text-sm font-semibold text-fg/70">Your prediction</span>
              )}
            </div>
            <div className="flex items-center gap-3">
              <span className="font-mono text-sm font-bold text-gold-deep">{fmtMoney(payout)}</span>
              <ChevronUp
                className={`h-4 w-4 text-fg/40 transition-transform duration-300 ${sheetOpen ? 'rotate-180' : ''}`}
              />
            </div>
          </button>

          {/* Drag handle */}
          <div className="absolute left-1/2 top-2 h-1 w-10 -translate-x-1/2 rounded-full bg-fg/20" />

          {/* Expanded content */}
          <div className="max-h-[75dvh] overflow-y-auto px-5 pb-6 [&::-webkit-scrollbar]:hidden">
            {BetPanelContent}
          </div>
        </div>
      </div>
